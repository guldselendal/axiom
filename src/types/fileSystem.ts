...
export const createNoteFile = async (fileName: string): Promise<{ success: boolean; filePath?: string; error?: string }> => {
  if (!window.electronAPI) {
    throw new Error('Electron API not available. This app requires Electron.')
  }
  return await window.electronAPI.createNoteFile(fileName)
}
