...
import { MarkdownNote, getNoteTypeFromFilePath } from './types/notes'
    ...
    if (savedNotes && Array.isArray(savedNotes) && savedNotes.length > 0) {
      ...
      const convertedNotes = await Promise.all(
        savedNotes.map(async (oldNote: any) => {
          ...
          const noteType = getNoteTypeFromFilePath(oldNote.filePath)
          ...
          if (note.filePath && !note.canvasId) {
            try {
              ...
              if (result.success && result.content !== undefined) {
                ...
                if (getNoteTypeFromFilePath(note.filePath) === 'excalidraw') {
                  try {
                    const excalidrawData = JSON.parse(result.content)
                    // Always use filename for Excalidraw titles (not stored title which might be outdated)
                    const titleFromFilename = note.filePath ? getTitleFromFilePath(note.filePath) : ''
                    note = {
                      ...note,
                      type: 'excalidraw',
                      excalidrawData,
