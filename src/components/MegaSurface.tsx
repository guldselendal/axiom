...
export default function MegaSurface({ canvasId = 'My Desk', notes: externalNotes, onNotesChange, onFileCreated, onFileRename, onFileDelete, onCreateNoteOnCanvas, zoom: externalZoom, onZoomChange, onCanvasChange, canvases = [] }: MegaSurfaceProps) {
  ...
  // Check both .md and .excalidraw.md files (and legacy .excalidraw)
  const getNextAvailableNoteNumber = useCallback(() => {
    const noteNumbers = new Set<number>();
    notes.forEach(note => {
      if (note.filePath) {
        // Match Note X.md, Note X.excalidraw.md, or Note X.excalidraw
        // Check longer extensions first (.excalidraw.md before .excalidraw)
        const match = note.filePath.match(/Note (\d+)\.(excalidraw\.md|excalidraw|md)$/);
        if (match) {
  ...
  const createNoteFromFile = useCallback(async (filePath: string, noteType: 'markdown' | 'excalidraw') => {
    ...
    try {
      ...
      if (noteType === 'excalidraw') {
        // For excalidraw, ensure we have the correct extension (.excalidraw)
        if (filePath.endsWith('.md') && !filePath.endsWith('.excalidraw.md')) {
          // Replace .md with .excalidraw (or keep .excalidraw.md if it exists for backward compatibility)
          if (filePath.endsWith('.excalidraw.md')) {
            actualFilePath = filePath; // Keep existing .excalidraw.md files
          } else {
          ...
          // Keep .excalidraw.md for backward compatibility
      ...
      // create it with default structure (uses .excalidraw)
      if (!result.success && noteType === 'excalidraw' && (actualFilePath.endsWith('.excalidraw.md') || actualFilePath.endsWith('.excalidraw'))) {
        ...
        if (noteType === 'excalidraw') {
          try {
            ...
            if (!trimmedContent.startsWith('{') && !trimmedContent.startsWith('[')) {
              ...
              const title = getTitleFromFilePath(actualFilePath)
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    // Load file content
    try {
      // For Excalidraw files, retry loading in case file hasn't been fully flushed to disk (production builds)
      const noteType = getNoteTypeFromFilePath(filePath);
      let result = await loadNoteFromFile(filePath);
      
      // Retry loading Excalidraw files if first attempt fails (handles file I/O timing in production)
      if (noteType === 'excalidraw' && !result.success) {
        let retries = 0;
        while (!result.success && retries < 3) {
          await new Promise(resolve => setTimeout(resolve, 200));
          result = await loadNoteFromFile(filePath);
          retries++;
        }
      }
      ...
      if (result.success && result.content !== undefined) {
        if (noteType === 'excalidraw') {
          ...
          try {
            console.log('handleDrop: Parsing Excalidraw file, content length:', result.content?.length);
            
            // Validate content is not empty
            if (!result.content || result.content.trim().length === 0) {
              console.error('handleDrop: Excalidraw file is empty! File path:', filePath);
              return;
            }
            
            const excalidrawData = JSON.parse(result.content) as ExcalidrawData;
            
            console.log('handleDrop: Parsed Excalidraw data:', {
              hasElements: !!excalidrawData.elements,
              elementsCount: excalidrawData.elements?.length,
              hasAppState: !!excalidrawData.appState,
              hasFiles: !!excalidrawData.files,
            });
            
            // Validate and ensure structure is correct
            if (!excalidrawData.elements) {
              console.warn('handleDrop: Excalidraw data missing elements array, initializing empty array');
              excalidrawData.elements = [];
            }
            if (!excalidrawData.appState) {
              excalidrawData.appState = { viewBackgroundColor: '#ffffff' };
            ...
            if (!Array.isArray(excalidrawData.elements)) {
              console.error('handleDrop: elements is not an array!', typeof excalidrawData.elements, excalidrawData.elements);
              excalidrawData.elements = [];
            }
            ...
            if (excalidrawData.elements.length === 0) {
              console.warn('handleDrop: Warning - Excalidraw file has no elements! File path:', filePath);
            }
            ...
            console.log('handleDrop: Created note with excalidrawData:', {
              noteId: newNote.id,
              elementsCount: newNote.excalidrawData.elements?.length,
              hasElements: !!newNote.excalidrawData.elements,
            });
            
            setNotes([...currentNotes, newNote]);
          } catch (parseError) {
            console.error('handleDrop: Error parsing Excalidraw file:', parseError);
            console.error('handleDrop: File content preview:', result.content?.substring(0, 500));
            console.error('handleDrop: File path:', filePath);
          }
        } else {
          // Markdown note
          const content = result.content || '';
            ...
            <ExcalidrawNoteEditor
              ...
              onSave={async (excalidrawData, newFilePath) => {
                ...
                if (filePath) {
                  ...
                  const currentNotes = notes || [];
                  const noteExistsOnCanvas = currentNotes.some((n: Note) => n.id === editingNote.note.id);
                    ...
                    const updatedNotes = currentNotes.map((n: Note) => {
                      if (n.id === editingNote.note.id && isExcalidrawNote(n)) {
                        ...
                        const updatedNote: ExcalidrawNote = {
                          ...
                          excalidrawData: {
                            ...excalidrawData,
                            elements: excalidrawData.elements.map((el: any) => ({ ...el })), // Deep clone each element
                            appState: { ...excalidrawData.appState },
                            files: excalidrawData.files ? { ...excalidrawData.files } : {},
                          },
                    ...
                    if (loadResult.success && loadResult.content) {
                      try {
                        ...
                        if (excalidrawData.elements && Array.isArray(excalidrawData.elements)) {
                          ...
                          const baseNote = (currentNote || editingNote.note) as ExcalidrawNote;
                          const updatedNote: ExcalidrawNote = {
                            ...baseNote,
                            type: 'excalidraw',
                            excalidrawData,
