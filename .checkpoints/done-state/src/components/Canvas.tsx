...
// Maximum canvas size for world coordinate space
// All positions are calculated in this coordinate space, then transformed for display
const MAX_CANVAS_SIZE = 1000000 // 1 million pixels in each direction
...
// Calculates in world coordinate space (MAX_CANVAS_SIZE) before applying transform
// screenX, screenY: viewport coordinates (clientX, clientY)
// rect: canvas element's bounding rect (accounts for header bar and any positioning)
...
const screenToWorldCenter = (screenX: number, screenY: number, rect: DOMRect, pan: { x: number; y: number }, zoom: number) => {
  // Convert screen coordinates to coordinates relative to canvas element
  // rect.left and rect.top account for the canvas element's position on screen
  ...
  // World coordinates are in the MAX_CANVAS_SIZE coordinate space
  // These coordinates are then transformed for display via CSS transform
  const worldCenterX = (canvasRelativeX - pan.x) / zoom
  const worldCenterY = (canvasRelativeY - pan.y) / zoom
...
const Canvas = forwardRef<CanvasHandle, CanvasProps>(({ zoom, onZoomChange, activeTool, canvasId, onFileCreated }, ref) => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    if (result.success && result.content !== undefined) {
      ...
      // Get canvas bounding rect for coordinate transformation
      ...
      // Calculate in world coordinate space (MAX_CANVAS_SIZE), then transform for display
      ...
      >
        <div
          style={{
            ...
            transformOrigin: '0 0',
            width: `${MAX_CANVAS_SIZE}px`,
            height: `${MAX_CANVAS_SIZE}px`,
            position: 'absolute',
            left: 0,
            top: 0,
          }}
