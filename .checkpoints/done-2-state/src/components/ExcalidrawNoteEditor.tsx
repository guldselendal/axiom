...
const ExcalidrawNoteEditor: React.FC<ExcalidrawNoteEditorProps> = ({
  ...
  // Store onSave in a ref so immediateSave doesn't need to be recreated when onSave changes
  const onSaveRef = useRef(onSave)
  const filePathRef = useRef(filePath)
  // Store excalidrawAPI in a ref so immediateSave can always access the latest API
  ...
  useEffect(() => {
    ...
    console.log('ExcalidrawNoteEditor: Refs updated, filePath:', filePath, 'onSave:', typeof onSave)
  }, [onSave, filePath])
  ...
  // This callback is stable and doesn't depend on onSave, filePath, or excalidrawAPI directly
  const immediateSave = useCallback(async (data: ExcalidrawData) => {
    const currentFilePath = filePathRef.current
    const currentAPI = excalidrawAPIRef.current
    console.log('🔵 ExcalidrawNoteEditor: immediateSave called!', {
      filePath: currentFilePath,
      elementsFromParam: data.elements?.length,
      apiAvailable: !!currentAPI,
      noteId: note.id,
      noteTitle: note.title
    })
    
    // Always get the latest state from Excalidraw API if available, to ensure we save the most current data
    let dataToSave = data
    if (currentAPI) {
      const latestElements = currentAPI.getSceneElements()
      const latestAppState = currentAPI.getAppState()
      const latestFiles = currentAPI.getFiles()
      
      console.log('ExcalidrawNoteEditor: Got latest state from API, elements:', latestElements.length)
      
      // Use the latest state from API instead of the passed data
      dataToSave = {
        type: 'excalidraw',
        version: 2,
        source: 'https://excalidraw.com',
        elements: latestElements.map(el => ({ ...el })),
        appState: { ...latestAppState },
        files: Object.fromEntries(
          Object.entries(latestFiles).map(([id, file]) => [
            id,
            {
              id: file.id,
      ...
      console.warn('ExcalidrawNoteEditor: excalidrawAPI not available, using passed data')
    }
    
    console.log('ExcalidrawNoteEditor: Final dataToSave, elements:', dataToSave.elements?.length)
    ...
    if (!currentFilePath) {
      console.error('🔴 ExcalidrawNoteEditor: Cannot save, no filePath!', {
        filePathRef: filePathRef.current,
    ...
    if (isSavingRef.current) {
      console.log('ExcalidrawNoteEditor: Save already in progress, queuing data with', dataToSave.elements.length, 'elements')
    ...
    try {
      console.log('ExcalidrawNoteEditor: Starting immediate save, elements count:', dataToSave.elements?.length, 'filePath:', currentFilePath)
      const jsonContent = JSON.stringify(dataToSave, null, 2)
      console.log('ExcalidrawNoteEditor: JSON content length:', jsonContent.length)
      const result = await saveNoteToFile(currentFilePath, jsonContent)
      console.log('ExcalidrawNoteEditor: Save result:', result)
      if (result.success) {
        console.log('ExcalidrawNoteEditor: Immediate save successful, calling onSave callback with', dataToSave.elements?.length, 'elements')
        // Only call onSave if save was successful - use ref to get latest callback
        onSaveRef.current(dataToSave, currentFilePath)
        console.log('ExcalidrawNoteEditor: onSave callback called')
      } else {
      ...
      if (pendingSaveRef.current && pendingSaveRef.current !== dataToSave) {
        ...
        console.log('ExcalidrawNoteEditor: Processing queued save')
  ...
  const handleExcalidrawChange = useCallback((
    ...
    const currentFilePath = filePathRef.current
    const currentAPI = excalidrawAPIRef.current
    console.log('🔵 ExcalidrawNoteEditor: handleExcalidrawChange called!', {
    ...
    // Skip save on initial load OR if filePath is not set yet
    if (isInitialLoadRef.current) {
      console.log('🔵 ExcalidrawNoteEditor: Skipping save on initial load, setting flag to false')
      isInitialLoadRef.current = false
      return
    }
    
    if (!currentFilePath) {
      console.warn('🔵 ExcalidrawNoteEditor: Skipping save, filePath not set yet')
      return
    }
    ...
    console.log('ExcalidrawNoteEditor: Calling immediateSave with', excalidrawData.elements.length, 'elements, filePath:', currentFilePath)
  ...
  const flushPendingSave = useCallback(async () => {
    ...
    if (dataToSave) {
      ...
      const result = await saveNoteToFile(currentFilePath, jsonContent)
      if (result.success) {
        // Only call onSave if there are elements to avoid updating note with empty data
        if (dataToSave.elements && dataToSave.elements.length > 0) {
          // Call onSave to update the note on canvas with latest data - use ref to get latest callback
          onSaveRef.current(dataToSave, currentFilePath)
          console.log('flushPendingSave: Save successful, called onSave with', dataToSave.elements.length, 'elements')
        } else {
          console.log('flushPendingSave: Save successful but no elements, skipping onSave callback')
        }
        pendingSaveRef.current = null
    ...
    return false
  }, [excalidrawAPI]) // Only depend on excalidrawAPI, not filePath or onSave
    ...
    return () => {
      ...
      if (excalidrawAPI) {
        ...
        if (currentFilePath) {
          ...
          saveNoteToFile(currentFilePath, jsonContent).then(result => {
            if (result.success) {
              currentOnSave(excalidrawData, currentFilePath)
            }
          })
        }
      } else if (pendingSaveRef.current && currentFilePath) {
        // Fallback to pending save if API not available
        const jsonContent = JSON.stringify(pendingSaveRef.current, null, 2)
        saveNoteToFile(currentFilePath, jsonContent).then(result => {
          if (result.success) {
            currentOnSave(pendingSaveRef.current!, currentFilePath)
          }
        })
      }
    }
  }, [excalidrawAPI]) // Only depend on excalidrawAPI, use refs for filePath and onSave
