...
const Sidebar = forwardRef<SidebarHandle, SidebarProps>(({ onOpenFile, onFileRename, onFileDelete, onCreateNoteOnCanvas, currentCanvas, onCanvasChange, canvases, onCanvasListUpdate, isHidden = false, onToggleHide: _onToggleHide, canvasNotes = [] }, ref) => {
  ...
  const loadFileTitles = useCallback(async (fileList: FileItem[]) => {
      ...
      fileList.forEach(file => {
        ...
        if (!titlesMap.has(file.path)) {
          if (file.name.endsWith('.excalidraw')) {
            ...
            const title = file.name.slice(0, -11) // Remove .excalidraw
    ...
    await Promise.all(fileList.map(async (file) => {
      try {
        ...
        const fallbackTitle = file.name.endsWith('.md') 
          ? file.name.slice(0, -3) 
    ...
    setFileTitles(prevTitles => {
      ...
      titlesMap.forEach((title, path) => {
        // Only update if we don't already have a manually set title
        if (!prevTitles.has(path)) {
  ...
  const handleFileDoubleClick = async (file: FileItem) => {
    ...
    const noteType = file.name.endsWith('.excalidraw') ? 'excalidraw' : 'markdown'
  ...
  const handleRename = async (file: FileItem) => {
    ...
    if (file.name.endsWith('.excalidraw')) {
      fileNameWithoutExt = file.name.slice(0, -11) // Remove .excalidraw
    } else if (file.name.endsWith('.md')) {
      fileNameWithoutExt = file.name.slice(0, -3) // Remove .md
    }
    // Ensure we don't have any extension left
    if (fileNameWithoutExt.endsWith('.excalidraw') || fileNameWithoutExt.endsWith('.md')) {
      // If somehow extension is still there, remove it again
      if (fileNameWithoutExt.endsWith('.excalidraw')) {
        fileNameWithoutExt = fileNameWithoutExt.slice(0, -11)
      } else if (fileNameWithoutExt.endsWith('.md')) {
        fileNameWithoutExt = fileNameWithoutExt.slice(0, -3)
      }
    }
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      ...
      if (originalFile) {
        ...
        const isExcalidraw = originalFile.name.endsWith('.excalidraw')
        const isMarkdown = originalFile.name.endsWith('.md')
        const originalNameWithoutExt = isExcalidraw
          ? originalFile.name.slice(0, -11) // Remove .excalidraw
          : isMarkdown
          ? originalFile.name.slice(0, -3) // Remove .md
          : originalFile.name
        ...
        const duplicateFile = files.find(f => {
          ...
          const otherIsExcalidraw = f.name.endsWith('.excalidraw')
          const otherIsMarkdown = f.name.endsWith('.md')
          const otherNameWithoutExt = otherIsExcalidraw
            ? f.name.slice(0, -11)
            : otherIsMarkdown
            ? f.name.slice(0, -3)
            : f.name
      ...
      if (result.success && result.newFilePath) {
        ...
        const originalFile = files.find(f => f.path === renamingFile)
        const isExcalidraw = originalFile?.name.endsWith('.excalidraw') ?? false
        const isMarkdown = originalFile?.name.endsWith('.md') ?? false
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const getNextAvailableNoteNumber = () => {
                        ...
                        files.forEach(file => {
                          const match = file.name.match(/^Note (\d+)\.(md|excalidraw)$/)
                          if (match) {
                  ...
                  <button
                    onClick={async () => {
                      ...
                      if (result.success && result.filePath) {
                        ...
                        // If the file was created with wrong extension, fix it
                        let finalFilePath = result.filePath;
                        if (result.filePath.endsWith('.md')) {
                          // The file was created with wrong extension, use .excalidraw instead
                          finalFilePath = result.filePath.replace(/\.md$/, '.excalidraw');
                          console.log('Sidebar: File was created with .md extension, using:', finalFilePath);
                        }
