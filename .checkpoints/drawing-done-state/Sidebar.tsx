...
import { saveCanvasList, saveCurrentCanvas, removeNoteFromAllCanvases, deleteCanvas } from '../utils/storage'
...
const Sidebar = forwardRef<SidebarHandle, SidebarProps>(({ onOpenFile, onFileRename, onFileDelete, onFileCreated, currentCanvas, onCanvasChange, canvases, onCanvasListUpdate, isHidden = false, onToggleHide: _onToggleHide, canvasNotes = [] }, ref) => {
  ...
  const [selectedFiles, setSelectedFiles] = useState<Set<string>>(new Set()) // Track selected files by path
  const [showAddMenu, setShowAddMenu] = useState(false) // Show dropdown for Add Note / Add Drawing
    ...
    await Promise.all(fileList.map(async (file) => {
      try {
        ...
        if (result.success && result.content) {
          ...
          const title = firstLine.trim() || (file.name.endsWith('.md') ? file.name.slice(0, -3) : file.name.endsWith('.excalidraw') ? file.name.slice(0, -11) : file.name)
          titlesMap.set(file.path, title)
        } else {
          // Fallback to filename if file is empty or can't be loaded
          const fallbackTitle = file.name.endsWith('.md') ? file.name.slice(0, -3) : file.name.endsWith('.excalidraw') ? file.name.slice(0, -11) : file.name.endsWith('.excalidraw') ? file.name.slice(0, -11) : file.name
  ...
  const handleSelectFile = (file: FileItem) => {
    ...
    setContextMenuFile(null) // Close context menu
  }
  ...
  const handleDelete = async (file: FileItem) => {
    ...
    const filesToDelete = selectedFiles.size > 0 
      ? Array.from(selectedFiles).map(path => files.find(f => f.path === path)).filter((f): f is FileItem => f !== undefined)
      : [file]
    
    // Confirm deletion
    const fileNames = filesToDelete.map(f => f.name).join(', ')
    const confirmMessage = filesToDelete.length === 1
      ? `Are you sure you want to delete "${fileNames}"? This action cannot be undone.`
      : `Are you sure you want to delete ${filesToDelete.length} files (${fileNames})? This action cannot be undone.`
    ...
    const deletePromises = filesToDelete.map(async (fileToDelete) => {
      const result = await deleteNoteFile(fileToDelete.path)
      if (result.success) {
        // Remove note from all canvases in storage
        await removeNoteFromAllCanvases(fileToDelete.path)
        
        // Notify Canvas to remove note immediately (if callback provided)
        if (onFileDelete) {
          onFileDelete(fileToDelete.path)
        }
        return { success: true, path: fileToDelete.path }
      } else {
        return { success: false, path: fileToDelete.path, error: result.error }
      }
    })
    ...
    // Show warning if any deletions failed
    const failedResults = results.filter(r => !r.success)
    if (failedResults.length > 0) {
      const errorMessages = failedResults.map(r => r.error || 'Failed to delete file').join(', ')
      setWarningMessage(`Failed to delete ${failedResults.length} file(s): ${errorMessages}`)
    }
  }
  ...
  return (
    <div className={`relative bg-white border-r border-gray-200 flex flex-col h-full overflow-y-auto transition-all duration-300 ease-in-out ${
              ...
              {showAddMenu && (
                ...
                >
                  <button
                    onClick={async () => {
                      setShowAddMenu(false)
                      console.log('➕ Sidebar: Add Note clicked, creating new markdown file...')
                      
                      // Find the next available note number
                      const getNextAvailableNoteNumber = () => {
                        // Extract all note numbers from existing files (both .md and .excalidraw)
                        const noteNumbers = new Set<number>()
                        files.forEach(file => {
                          const mdMatch = file.name.match(/^Note (\d+)\.md$/)
                          const excMatch = file.name.match(/^Note (\d+)\.excalidraw$/)
                          if (mdMatch) {
                            noteNumbers.add(parseInt(mdMatch[1], 10))
                          }
                          if (excMatch) {
                            noteNumbers.add(parseInt(excMatch[1], 10))
                          }
                        })
                  ...
                  <button
                    onClick={async () => {
                      setShowAddMenu(false)
                      console.log('➕ Sidebar: Add Drawing clicked, creating new excalidraw file...')
                      ...
                      if (result.success && result.filePath) {
                        ...
                        // Open Excalidraw editor for the new file
                        // Wait a bit for file to be fully written
                        await new Promise(resolve => setTimeout(resolve, 200))
                        if ((window as any).__openEditorForFile) {
                          (window as any).__openEditorForFile(result.filePath, 'excalidraw')
                        } else {
                          console.error('Sidebar: __openEditorForFile is not available')
                        }
                      }
                    }}
                    className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-2"
                ...
                return (
                  <div key={`${file.path}-${file.name}`} className="relative">
                    ...
                    <button
                      ...
                      } ${selectedFiles.has(file.path) ? 'bg-blue-50 border border-blue-200' : ''}`}
                    >
                      {selectedFiles.size > 0 && (
                        <input
                          type="checkbox"
                          ...
                          onClick={(e) => e.stopPropagation()}
                          className="w-4 h-4 text-primary-600 rounded border-gray-300 focus:ring-primary-500 flex-shrink-0"
                        />
                      )}
                      ...
                      <span className="text-sm flex-1 break-words">
                        {fileTitles.get(file.path) || (file.name.endsWith('.md') ? file.name.slice(0, -3) : file.name.endsWith('.excalidraw') ? file.name.slice(0, -11) : file.name)}
                      </span>
                    </button>
                    )}
                  </div>
      ...
      {contextMenuFile && (
        ...
        >
          <button
            onClick={() => handleSelectFile(contextMenuFile)}
            className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
