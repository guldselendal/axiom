...
export default function MegaSurface({ canvasId = 'My Desk', notes: externalNotes, onNotesChange, onFileCreated, onFileRename, onFileDelete, zoom: externalZoom, onZoomChange, onCanvasChange, canvases = [] }: MegaSurfaceProps) {
  ...
  const createNoteAtPosition = useCallback(async (screenX: number, screenY: number, noteType: 'markdown' | 'excalidraw' = 'markdown') => {
    ...
    const createResult = await createNoteFile(fileName, noteType);
  ...
  // Function to open editor for a file (can be called programmatically)
  const openEditorForFile = useCallback(async (filePath: string, noteType: 'markdown' | 'excalidraw' = 'markdown') => {
    // Check if already editing this file
    const normalizedFilePath = filePath.split('/').pop() || filePath;
    if (editingNotes.some(e => {
      const ePath = e.filePath?.split('/').pop() || e.filePath;
      return ePath === normalizedFilePath;
    })) {
      console.log('Editor already open for file:', filePath);
      return;
    }
    ...
    try {
        ...
        if (noteType === 'excalidraw') {
          ...
          try {
            ...
            if (!content.trim() || !content.trim().startsWith('{')) {
              // File is empty or doesn't have valid JSON - create default structure
              console.log('Excalidraw file is empty or invalid, creating default structure');
              excalidrawData = {
                type: 'excalidraw',
                version: 2,
                source: 'https://excalidraw.com',
            ...
            const tempNote: ExcalidrawNote = {
              ...
              width: 200,
              height: 150,
              title: filePath.endsWith('.excalidraw') ? filePath.split('/').pop()?.slice(0, -11) : filePath.split('/').pop(),
              color: '#ffffff',
              filePath: filePath,
            ...
            setEditingNotes(prev => [...prev, {
              note: tempNote,
              position: { x: centerX - 210, y: centerY - 250 },
              filePath: filePath,
            }]);
          } catch (parseError) {
            console.error('Error parsing Excalidraw file:', parseError);
            // Create a default Excalidraw structure if parsing fails
            const defaultData = {
              type: 'excalidraw',
              version: 2,
              source: 'https://excalidraw.com',
            ...
            try {
              const jsonContent = JSON.stringify(defaultData, null, 2);
              await saveNoteToFile(filePath, jsonContent);
            } catch (saveError) {
              console.error('Error saving default Excalidraw structure:', saveError);
            }
      ...
      console.error('Error loading file for editor:', error);
    }
  }, [editingNotes, getViewportRect, loadNoteFromFile]);
  
  // Expose function globally for Sidebar to call
  useEffect(() => {
    (window as any).__openEditorForFile = openEditorForFile;
    return () => {
      delete (window as any).__openEditorForFile;
    };
  }, [openEditorForFile]);
  ...
  const noteLinks = useMemo(() => {
    ...
    notes.forEach(fromNote => {
      // Only markdown notes have content for link parsing
      if (!isMarkdownNote(fromNote)) return;
      
      // Check if note has content - if not, log for debugging
      if (!fromNote.content) {
        console.log('Note without content (cannot calculate links):', {
          id: fromNote.id,
          title: fromNote.title,
          filePath: fromNote.filePath,
          canvasId: fromNote.canvasId,
        });
        return;
      }
      ...
      linkMatches.forEach(link => {
        // Normalize the linked file name
        const linkedFileName = link.fileName.trim();
        const normalizedLinkedFileName = linkedFileName.endsWith('.md') 
          ? linkedFileName.slice(0, -3) 
          : linkedFileName;
        ...
        if (!targetNote) {
          targetNote = notes.find(toNote => {
            ...
            // Normalize the target note's file path
            const targetFileName = toNote.filePath.split('/').pop() || toNote.filePath;
            const normalizedTargetFileName = targetFileName.endsWith('.md')
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {noteLinks.map((link, index) => {
            ...
            // Calculate the intersection point with the note card edge
            // Find where the line from center intersects the card rectangle
            let arrowEndX = toCenterX;
            let arrowEndY = toCenterY;
            
            // Calculate intersection with vertical edges
            if (Math.abs(unitX) > 0.001) {
              const tLeft = (link.to.worldX - toCenterX) / unitX;
              const tRight = (link.to.worldX + link.to.width - toCenterX) / unitX;
              const tVertical = unitX > 0 ? tRight : tLeft;
              
              if (tVertical > 0) {
                const yAtEdge = toCenterY + unitY * tVertical;
                if (yAtEdge >= link.to.worldY && yAtEdge <= link.to.worldY + link.to.height) {
                  arrowEndX = toCenterX + unitX * tVertical;
                  arrowEndY = yAtEdge;
                }
              }
            }
            ...
            if (Math.abs(unitY) > 0.001) {
              const tTop = (link.to.worldY - toCenterY) / unitY;
              const tBottom = (link.to.worldY + link.to.height - toCenterY) / unitY;
              const tHorizontal = unitY > 0 ? tBottom : tTop;
              
              if (tHorizontal > 0) {
                const xAtEdge = toCenterX + unitX * tHorizontal;
                if (xAtEdge >= link.to.worldX && xAtEdge <= link.to.worldX + link.to.width) {
                  const candidateY = toCenterY + unitY * tHorizontal;
                  // Use this if it's closer to the target
                  const distToCandidate = Math.sqrt((candidateY - toCenterY) ** 2 + (xAtEdge - toCenterX) ** 2);
                  const distToCurrent = Math.sqrt((arrowEndY - toCenterY) ** 2 + (arrowEndX - toCenterX) ** 2);
                  if (distToCandidate < distToCurrent || arrowEndX === toCenterX) {
                    arrowEndX = xAtEdge;
                    arrowEndY = candidateY;
                  }
                }
              }
            }
            
            // Calculate arrow start point (start from note edge, not center)
            let arrowStartX = fromCenterX;
            let arrowStartY = fromCenterY;
            
            // Find intersection with source card edges
            if (Math.abs(unitX) > 0.001) {
              const tLeft = (link.from.worldX - fromCenterX) / unitX;
              const tRight = (link.from.worldX + link.from.width - fromCenterX) / unitX;
              const tVertical = unitX < 0 ? tRight : tLeft;
              
              if (tVertical > 0) {
                const yAtEdge = fromCenterY + unitY * tVertical;
                if (yAtEdge >= link.from.worldY && yAtEdge <= link.from.worldY + link.from.height) {
                  arrowStartX = fromCenterX + unitX * tVertical;
                  arrowStartY = yAtEdge;
                }
              }
            }
            ...
            if (Math.abs(unitY) > 0.001) {
              const tTop = (link.from.worldY - fromCenterY) / unitY;
              const tBottom = (link.from.worldY + link.from.height - fromCenterY) / unitY;
              const tHorizontal = unitY < 0 ? tBottom : tTop;
              
              if (tHorizontal > 0) {
                const xAtEdge = fromCenterX + unitX * tHorizontal;
                if (xAtEdge >= link.from.worldX && xAtEdge <= link.from.worldX + link.from.width) {
                  const candidateY = fromCenterY + unitY * tHorizontal;
                  const distToCandidate = Math.sqrt((candidateY - fromCenterY) ** 2 + (xAtEdge - fromCenterX) ** 2);
                  const distToCurrent = Math.sqrt((arrowStartY - fromCenterY) ** 2 + (arrowStartX - fromCenterX) ** 2);
                  if (distToCandidate < distToCurrent || arrowStartX === fromCenterX) {
                    arrowStartX = xAtEdge;
                    arrowStartY = candidateY;
                  }
                }
              }
            }
      ...
      {contextMenuNote && (
        ...
        >
          <button
            onClick={() => handleRemoveFromCanvas(contextMenuNote)}
            className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {editingNotes.map((editingNote) => {
            ...
            <ExcalidrawNoteEditor
              ...
              onSave={async (excalidrawData, newFilePath) => {
                const updatedNote: ExcalidrawNote = {
                  ...editingNote.note,
                  excalidrawData: excalidrawData,
                  title: editingNote.note.title || 'Excalidraw Drawing',
                };
                
                // Handle file rename if needed
                let finalFilePath = editingNote.filePath || newFilePath;
                if (newFilePath && newFilePath !== editingNote.filePath) {
                  finalFilePath = newFilePath;
                  updatedNote.filePath = newFilePath;
                  
                  if (onFileRename && editingNote.filePath) {
                    onFileRename(editingNote.filePath, newFilePath, updatedNote.title || 'Excalidraw Drawing');
                  }
                }
                ...
                if (finalFilePath) {
                  const jsonContent = JSON.stringify(excalidrawData, null, 2);
                  await saveNoteToFile(finalFilePath, jsonContent);
                }
                
                // Update note in canvas
                setNotes(notes.map(n => 
        ...
        // Default to MarkdownNote editor (HoverEditor)
        ...
        const editorNote = {
          ...
          content: isMarkdownNote(editingNote.note) ? editingNote.note.content : '',
        ...
        return (
          <HoverEditor
            ...
            filePath={editingNote.filePath}
            allNotes={notes.filter(isMarkdownNote)}
            canvases={canvases}
            onPositionChange={(newPosition) => {
              ...
              if (!linkedNote) {
                try {
                  ...
                  if (loadResult.success && loadResult.content !== undefined) {
                    ...
                    const createResult = await createNoteFile(normalizedFileName.endsWith('.md') ? normalizedFileName.slice(0, -3) : normalizedFileName, 'markdown');
                    if (createResult.success && createResult.filePath) {
                      const rect = getViewportRect();
                      const currentNoteScreenPos = worldToScreen(editingNote.note.worldX, editingNote.note.worldY, rect, camera);
            ...
            onSave={async (content, newFileName) => {
              if (!isMarkdownNote(editingNote.note)) {
                console.error('MegaSurface: Cannot save non-markdown note with HoverEditor');
                return;
              }
